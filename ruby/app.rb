...
class App < Sinatra::Base
  ...
  get '/api/chair/search' do
    ...
    limit_offset = " ORDER BY rev_popularity ASC, id ASC" # XXX: mysql-cs-bind doesn't support escaping variables for limit and offset
  ...
  post '/api/chair/buy/:id' do
    ...
    transaction('post_api_chair_buy') do |tx_name|
      chair = db.xquery('SELECT * FROM chair WHERE id = ? FOR UPDATE', id).first
      if !chair || chair[:stock] == 0
        rollback_transaction(tx_name) if in_transaction?(tx_name)
        halt 404
      end
      stock = chair[:stock] - 1
      db.xquery('UPDATE chair SET stock = ? WHERE id = ?', stock, id)
    end
  ...
  get '/api/estate/search' do
    ...
    search_condition = search_queries.join(' AND ')
    limit_offset = " ORDER BY rev_popularity ASC, id ASC" # XXX:
    ...
    estates = estates.slice(per_page * page, per_page) || []

    { count: estates.size, estates: estates.map { |e| camelize_keys_for_estate(e) } }.to_json
  end
  ...
  post '/api/estate/nazotte' do
    ...
    sql = 'SELECT * FROM estate WHERE latitude <= ? AND latitude >= ? AND longitude <= ? AND longitude >= ?'
    ...
    estates = estates.sort_by { |estate| [estate[:rev_popularity], estate[:id] ]}
    estates_in_polygon = []
    # conds = []
    # ids = []
