...
class App < Sinatra::Base
  ...
  get '/api/chair/search' do
    ...
    limit_offset = " ORDER BY popularity DESC, id DESC LIMIT #{per_page} OFFSET #{per_page * page}" # XXX: mysql-cs-bind doesn't support escaping variables for limit and offset
  ...
  get '/api/estate/search' do
    ...
    search_condition = search_queries.join(' AND ')
    limit_offset = " ORDER BY popularity DESC, id DESC LIMIT #{per_page} OFFSET #{per_page * page}" # XXX:
  ...
  post '/api/estate/nazotte' do
    ...
    sql = 'SELECT * FROM estate WHERE latitude <= ? AND latitude >= ? AND longitude <= ? AND longitude >= ? ORDER BY popularity DESC, id DESC'
  ...
  get '/api/recommended_estate/:id' do
    ...
    sql = "SELECT * FROM estate WHERE (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) ORDER BY popularity DESC, id DESC LIMIT #{LIMIT}" # XXX:
